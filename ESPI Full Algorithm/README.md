...
- **Difference amplification**: None (raw differences), Normalize contrast (OpenCV NORM_MINMAX), Gain factor (multiply by gain_factor setting, default), CLAHE (Contrast Limited Adaptive Histogram Equalization, with Clip Limit and Tile Grid Size controls), or Gamma correction (non-linear brightness remapping via a lookup table, with a Gamma control). Only one method's parameter controls are shown at a time, matching whichever radio button is selected. All five methods only ever transform the frame subtraction display, never the Live Feed.
