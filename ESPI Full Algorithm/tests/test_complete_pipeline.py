...
# sweep safely between frequencies, without complete_pipeline.py needing to
# know a GUI exists. Checked once per frequency, before any signal generator
# or camera command for that frequency is issued.
# ===========================================================================
...
class TestStopCheck:
    ...
    def test_true_before_first_frequency_returns_no_results(self, hw):
        result = cp.frequency_sweep(100, 300, 100, 2, 10000, 0.0, str(hw["tmp_path"]),
    ...
    def test_cleanup_still_happens_when_stopped(self, hw):
        with patch("complete_pipeline.turn_off_output") as mock_off, \
             patch("complete_pipeline.disconnect_camera") as mock_disc:
            cp.frequency_sweep(100, 300, 100, 2, 10000, 0.0, str(hw["tmp_path"]),
                                stop_check=lambda: True)
        mock_off.assert_called_once()
        mock_disc.assert_called_once()

    def test_reference_sweep_true_before_first_frequency_returns_no_results(self, hw):
        result = cp.reference_frequency_sweep(100, 300, 100, 2, 10000, 0.0, str(hw["tmp_path"]),
                                               stop_check=lambda: True)
        assert result == {}
    ...
    def test_reference_sweep_cleanup_still_happens_when_stopped(self, hw):
        with patch("complete_pipeline.turn_off_output") as mock_off, \
             patch("complete_pipeline.disconnect_camera") as mock_disc:
            cp.reference_frequency_sweep(100, 300, 100, 2, 10000, 0.0, str(hw["tmp_path"]),
                                          stop_check=lambda: True)
        mock_off.assert_called_once()
        mock_disc.assert_called_once()
